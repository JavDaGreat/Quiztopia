...
function QuizShow({
    ...
    questions.forEach((q) => {
      if (isNaN(Number(q.location.longitude))) {
        ...
        let marker = new mapboxgl.Marker()
          .setLngLat([
            Number(q.location.longitude),
            Number(q.location.latitude),
          ])
          .addTo(map)
          .setPopup(
  ...
  const handleDelete = async () => {
    const resp = await fetch(
      `https://fk7zu3f4gj.execute-api.eu-north-1.amazonaws.com/quiz/${name}`,
      {
        method: "DELETE",
  ...
  const handleAddquestion = () => {
    if (setAddQuiz && setEdit && setQuizName) setAddQuiz(true), setEdit(true);
    if (setQuizName) {
      setQuizName(name);
    }
  };
  ...
  return (
    <details className="p-4   bg-gray-200 max-w-lg mx-auto my-4">
      <summary className="flex justify-between items-center">
        <BsChevronDown className="hover:cursor-pointer" />{" "}
        ...
        {CanBeDeleted && (
          <div>
            <button
              ...
              className="bg-red-600 hover:bg-red-700 p-2 mx-4 my-1 rounded-md text-white"
            >
              Delete
            </button>
      ...
      <button
        className="bg-gray-700 hover:bg-black p-1 m-1 rounded-md text-white"
